...
BOT_TOKEN = "7441857309:AAGmDAUaGpIlNMZ4yQeNZggN1Hc4PuXTRSg"
OWNER_ID = 8022012230  # Replace with your Telegram ID
...
async def generate_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.message.chat_id
    api_key = get_user_apikey(chat_id)

    if not api_key:
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🌐 Converso AI Dashboard", url="https://conversoai.stylefort.store")]
        ])
        await update.message.reply_text(
            "🚀 *Set your API key to unlock premium image generation!*\n\n"
            "Follow these steps:\n"
            "🔹 Open [Converso AI Dashboard](https://conversoai.stylefort.store)\n"
            "🔹 Click *Get API Key* → enter WhatsApp number & OTP\n"
            "🔹 Click *Connect* → Go to *Settings* → *API Settings*\n"
            "🔹 Copy your API key → `/setkey <your_api_key>`",
            parse_mode="Markdown",
    ...
    try:
        ...
        await update.message.reply_photo(
            ...
            caption=f"🖼️ *Your image for:* `{prompt}`",
            parse_mode="Markdown",
            has_spoiler=True,
            reply_to_message_id=update.message.message_id
        )
